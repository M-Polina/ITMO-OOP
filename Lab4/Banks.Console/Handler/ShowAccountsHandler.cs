...
public class ShowAccountsHandler : AbstractHandler
{
    ...
    public override void Handle(string? request)
    {
        if (string.IsNullOrEmpty(request))
            return;
        if (request.Equals("Show accounts"))
            DoBusinessLogic();
    ...
    private void DoBusinessLogic()
    {
        ...
        foreach (var acc in client.AccountsList)
        {
            ...
            if (acc is DebitAccount)
                type = "Debit Account";
            else if (acc is CreditAccount)
                type = "Credit Account";
            else
                type = "Deposit Account";

            System.Console.WriteLine($"{type} : {acc.Id} : {acc.Money}");
        }
    }
